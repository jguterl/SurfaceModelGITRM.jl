...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov 15 17:01:06 2023
...
class GeomInput(GeomGroup):
    ...
    def SetInputPoints(self, GeoDim='3D'):
        for j, s in enumerate(['x', 'y', 'z']):
            for i in range(3):
                if self.Verbose:
                    print('setting "{}{}"'.format(s, i+1))
                self.GeomInput['{}{}'.format(
                    s, i+1)] = self.Triangles[:, i, j].squeeze()

    def SetAttr(self, Attribute, Value):
        if self.GeomInput.get(Attribute) is None:
            print(
                'Scalar attribute "{}" not found in GeomInput ... Creating a new scalar')
    ...
    def SetElemAttr(self, GroupID, Attribute, Value):
        ...
        if self.GeomInput.get(Attribute) is None:
            if GroupID == []:
                print(
                    'Element attribute "{}" not found in GeomInput ... Creating a new array for all elements')
                self.GeomInput[Attribute] = np.full((self.NElems), Value)
            else:
                raise ValueError(
                    'Cannot create a new attribute for specific groups. GroupID must be []')
        else:
...
class GeomSetup(GeomInput, GeomPlot):
    ...
    def CheckGeomInput(self, GeoDim='3D'):
        ...
        Defaults = self.DefaultsAttr[GeoDim]
        Requirements = list(Defaults['Vector'].keys()) + \
            list(Defaults['Scalar'].keys())

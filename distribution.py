...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov 20 21:31:49 2023
...
class Distribs():
    ...
    def GetPdf(cls, f: str or Callable[np.array, np.ndarray], **kwargs) -> (np.ndarray, np.ndarray):
        ...
        if type(f) == str:
            ...
            if kwargs.get('x') is None:
                raise KeywordError(
                    'Must provide a vector of x values for the function f tp get f(x)')
            else:
                x = kwargs.pop('x')
...
class ParticleDistribution():
    ...
    def PlotAttr(self, Attr=[], NewFig=True):
        if type(Attr) != list:
            Attr = [Attr]
        if Attr == []:
            Attr = [A for A, V in self.Particles.items() if type(V)
                    == np.ndarray]
        if NewFig:
            plt.figure()
        for A in Attr:
            assert A in list(self.Particles.keys()), 'Cannot find "{}" among list of attributes available:{}'.format(
                A, list(self.Particles.keys()))
            v = self.Particles[A]
    ...
    def AddAttr(self, Attr):
        assert self.Particles.get(
            Attr) is None, 'Attribute {} already exists.'.format(Attr)
    ...
    def SetAttr(self, Attr: list or str, f: float or np.ndarray or str or int, **kwargs):
        ...
        for A in Attr:
            ...
            if type(f) == float or type(f) == int:
                ...
                self.Particles[A] = self.Generate(
                    self.Particles['Np'], f, **kwargs)
